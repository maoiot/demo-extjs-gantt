    ...
    createScheduler: function () {
        ...
        var g = Ext.create("Sch.panel.SchedulerGrid", {
            ...
            resizeValidatorFn: function (resourceRecord, eventRecord, start, end) {
                if (eventRecord.get('Group') === 'min-one-day' && Sch.util.Date.getDurationInDays(start, end) < 1) {
                    return {
                        valid  : false,
                        message: 'Event must be at least one day long'
                    };
                }
            },

            dndValidatorFn: function (dragEventRecords, targetRowRecord) {
                if (!targetRowRecord.get('Available')) {
                    return {
                        valid  : false,
                        message: targetRowRecord.getName() + ' is not available.'
                    };
                }
            },
