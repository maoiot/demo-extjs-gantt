...
//            },
    ...
    show : function (eventRecord) {
        if (!eventRecord) return; // Could be called by Ext from internal ext code with no eventRecord (syncHidden behavior)

        var resource = eventRecord.getResource() || this.resourceRecord;

        // Load the image of the resource
        this.img.setSrc(resource.get('ImgUrl'));
