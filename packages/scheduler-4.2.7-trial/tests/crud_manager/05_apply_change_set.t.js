StartTest(function(t) {
    ...
    t.it('Should apply removed records correctly', function (t) {
        ...
        t.willFireNTimes(someStore1, 'datachanged', 4, 'datachanged fired proper number of times');

        response.someStore1.removed = [];

        var rows    = someStore1.getRange();

        crud.applySyncResponse(response);

        t.isDeeply(someStore1.getRange(), rows, 'Records not deleted');

        // ##12,13,14 imitate server driven removal
        response.someStore1.removed = [{ id : 11 }, { id : 12 }, { id : 13 }, { id : 14 }];
        response.someStore1.rows = [];

        crud.applySyncResponse(response);

        t.isDeeply(someStore1.getRemovedRecords(), [], 'Store.removed is empty');
        t.is(someStore1.getCount(), 0, 'Store is empty');
    });
