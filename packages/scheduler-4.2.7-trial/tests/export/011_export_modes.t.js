StartTest(function(t) {
    ...
    var plugin      = Ext.create('Sch.plugin.Export', {
        ...
        test                : true,
        tpl                 : '{header}tpl-body {HTML}{footer}',
        headerTpl           : '{foo}',
        headerTplDataFn     : function () {
            return { foo : 'foo!' };
        },
        footerTpl           : '{bar}',
        footerTplDataFn     : function () {
            return { bar : 'bar!' };
        }
    });
    ...
    t.waitForRowsVisible(scheduler, function () {
        ...
        t.it('"Export range=`complete`"', function (t) {
            ...
            plugin.doExport({
                ...
                t.ok(htmls[0].html.indexOf("foo!") > -1, "Page #0 has header");
                t.ok(htmls[1].html.indexOf("foo!") > -1, "Page #1 has header");

                t.ok(htmls[0].html.indexOf("tpl-body") > -1, "Page #0 has `tpl` part");
                t.ok(htmls[1].html.indexOf("tpl-body") > -1, "Page #1 has `tpl` part");

                t.ok(htmls[0].html.indexOf("bar!") > -1, "Page #0 has footer");
                t.ok(htmls[1].html.indexOf("bar!") > -1, "Page #1 has footer");
        ...
        t.it('"Export range=`complete` w/o header visible"', function (t) {
            ...
            plugin.doExport({
                ...
                t.is(htmls.length, 2, "2 pages exported");
                t.ok(htmls[1].html.indexOf("Mon 10 Jan") > -1, "Has column for `Mon 10 Jan`");
                t.is(htmls[0].html.indexOf("foo!"), -1, "Page #0 has no header");
                t.is(htmls[1].html.indexOf("foo!"), -1, "Page #1 has no header");
        ...
        t.it("Export range=`dates` (from 2011-01-03 till 2011-01-05)", function (t) {
            ...
            plugin.doExport({
                ...
                t.is(htmls[0].html.indexOf("bar!"), -1, "Page #0 has no footer");
        ...
        t.it("Export multipage vertical range=`complete`", function (t) {
        ...
        t.it("Export multipage vertical range=`dates` (from 2011-01-03 till 2011-01-05)", function (t) {
        ...
        t.it('`exporters` config supports definition using xclass', function (t) {
            ...
            var plugin      = Ext.create('Sch.plugin.Export', {
                ...
                printServer     : 'something',
                test            : true,
                exporters       : [{ xclass : 'Sch.plugin.exporter.MultiPage', iterateTimeout : 30 }, 'Sch.plugin.exporter.SinglePage']
            });
            ...
            t.is(exporters[0].iterateTimeout, 30, 'Correct iterateTimeout');
