StartTest(function (t) {
    ...
    var setup = function (config) {
        ...
        scheduler = t.getScheduler(Ext.apply({
            ...
            eventRenderer : function (eventRec, resourceRec, templateData) {
                 templateData.cls = 'sch-' + eventRec.getResourceId();
            }
        }, config));
    ...
    t.it('Resize should work correct w/o snapToIncrement', function (t) {
        ...
        t.chain(
            { click : '.sch-r1' },
            function (next) {
                t.selectorExists('.sch-r1.sch-event-selected', 'Selection is visible');
                next();
            },
            // resize start
            { drag: ".sch-r1 .sch-resizable-handle-start", by : [-5, -80] },
            // resize start
            function (next) {
                t.selectorExists('.sch-r1.sch-event-selected', 'Selection is restored');
                t.is(event.getResourceId(), 'r1', 'Resource is correct');
                t.is(event.getStartDate(), new Date(2014, 4, 28), 'Start date is correct');
                next();
            },
            // resize end
            { drag : ".sch-r1 .sch-resizable-handle-end", by : [-2, 39] },
            function (next) {
                // todo check dates
                t.is(event.getResourceId(), 'r1', 'Resource is correct');
            ...
            { drag : ".sch-r2 .sch-resizable-handle-end", by : [0, 40] },
            function (next) {
                // todo check dates
