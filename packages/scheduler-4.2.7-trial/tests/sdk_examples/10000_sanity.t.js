...
StartTest(function(t) {
    ...
    t.waitForSelector('.sch-timetd', function() {
        ...
        t.waitForComponentQueryNotFound('loadmask{isVisible()}', function(){
            t.monkeyTest(scheduler, 10, function (actionLog) {
                var suspendedComponents = t.cq('component{isLayoutSuspended()}');

                // only report in case of failure
                if (suspendedComponents.length > 0 || Ext.AbstractComponent.layoutSuspendCount > 0) {
                    t.fail('Monkey actions log: ' + JSON.stringify(actionLog));
                }
            });
